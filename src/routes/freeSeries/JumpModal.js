...
const JumpModal = ({
  ...
  const handleSelect = (record) => {
    ...
    dispatch({type:'freeSeries/setInputVal',payload:record})
  };
  const searchJump = (value) => {
    ...
    dispatch({type: 'freeSeries/getJumpData', payload: {opts}});
  };
  return (
    <Modal {...jumpModalProps} title="选择内容">
      ...
      <Table rowKey='id' size='small' columns={tableColum} dataSource={freeSeries.jumpData}
             loading={loading.effects['freeSeries/getJumpData']}></Table>
    </Modal>
  )
};
JumpModal.propTypes = {
