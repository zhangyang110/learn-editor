...
const JumpModal = ({
  ...
  const handleSelect = (record) => {
    //setInputVal(record.id,record.cover_url,record.cover_url);
    //dispatch({type: 'subjectSeries/public', payload: {jumpModalVisible: false}})
    dispatch({type:'subjectSeries/setInputVal',payload:record})
  }
  const searchJump = (value) => {
    ...
    dispatch({type: 'subjectSeries/getJumpData', payload: {opts}});
  };
  return (
    <Modal {...jumpModalProps} title="选择内容">
      ...
      <Table rowKey='id' size='small' columns={tableColum} dataSource={subjectSeries.jumpData}
             loading={loading.effects['subjectSeries/getJumpData']}></Table>
    </Modal>
  )
};
JumpModal.propTypes = {
